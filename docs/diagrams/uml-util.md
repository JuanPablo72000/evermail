...
3. Token/content handling uses AES-256-GCM. The AES key is stored and retrieved from the OS keyring (not derived from a master password, to avoid the unnecessary computational cost of a KDF in this OAuth-based authentication flow). Every failure in `SecurityUtil` — key generation, keyring read/write, or encrypt/decrypt — is reported as `CryptoException` (`ErrorCode.CRYPTO_OPERATION_FAILED`), never as `OAuthAuthenticationException`: these operations are local and unrelated to the OAuth handshake itself, even though the keys they protect originated from an OAuth login.
